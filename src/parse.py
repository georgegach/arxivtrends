...
def getDetails(path):
    with open(path, 'rb') as pdf:
        ...
        details = {}
        details['keywords'] = pdf.getDocumentInfo()['/Keywords']
...
def humanReadableDate(date):
    ...
    if delta == 0:
        return f"today in {date.strftime('%b %d, %Y')}"
    if delta == 1:
        return f"yesterday in {date.strftime('%b %d, %Y')}"
    if delta < 30:
...
def convert(fname, pages=None):
    ...
    convertedPDF = convertedPDF.replace('\n',' ')
